...
import { containsAnyKeyword, createBriefSummary, normalizeEmailText } from '../tools/emailTextTools';
...
export class ClassificationSkill {
  execute(email: SimpleEmail): EmailClassificationResult {
  ...
  evaluateImportance(email: SimpleEmail): EmailImportanceResult {
    const text = normalizeEmailText(email);
    const hasImportantSignal = containsAnyKeyword(text, IMPORTANT_KEYWORDS);
  ...
  summarize(email: SimpleEmail): EmailSummaryResult {
    return {
      summary: createBriefSummary(email)
    };
  }

  private detectCategory(text: string): EmailCategory {
    const orderedCategories: EmailCategory[] = ['spam', 'work', 'shopping', 'marketing', 'personal'];
    return orderedCategories.find((category) => containsAnyKeyword(text, CATEGORY_KEYWORDS[category])) || 'other';
  }

  private buildReasoning(category: EmailCategory): string {
